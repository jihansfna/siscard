<x-app-layout>
    <x-slot name="title">Feedback Management</x-slot>

    <section class="text-gray-800 dark:text-gray-200 transition-colors">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Feedback Management</h2>
                    ...
                    <thead class="bg-gray-50 dark:bg-[#1A1A1A] text-gray-700 dark:text-gray-300 text-xs uppercase font-semibold border-b border-gray-200 dark:border-gray-700/50">
                        <tr>
                            <th class="px-4 py-3 w-10">
                                <input type="checkbox" id="selectAllFeedbacks" class="rounded border-gray-300 dark:border-gray-600 dark:bg-[#242424] text-primary-600 focus:ring-primary-500" onclick="toggleSelectAllFeedbacks(this)">
                            </th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                        @forelse($feedbacks as $index => $fb)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-[#2A2A2A] transition-colors">
                                <td class="px-4 py-3">
                                    <input type="checkbox" name="ids[]" value="{{ $fb->id }}" form="bulkDeleteForm" class="feedback-checkbox rounded border-gray-300 dark:border-gray-600 dark:bg-[#242424] text-primary-600 focus:ring-primary-500" onclick="updateBulkDeleteBar()">
                                </td>
                                <td class="px-4 py-3 text-gray-500 dark:text-gray-400">{{ $feedbacks->firstItem() + $index }}</td>
                                <td class="px-4 py-3">
                                    <div class="font-bold text-gray-800 dark:text-white">{{ $fb->member->employee->name ?? 'Unknown' }}</div>
                                    <div class="text-xs text-gray-500">{{ $fb->member->employee->badge ?? '-' }}</div>
                                </td>
                                <td class="px-4 py-3 max-w-xs truncate" title="{{ $fb->description }}">
                                    {{ $fb->description }}
                                    @if($fb->remark)
                                        <div class="text-xs text-primary-600 mt-1 font-medium truncate" title="Reply: {{ $fb->remark }}">Reply: {{ $fb->remark }}</div>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400">Waiting</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="flex items-center gap-1">
                                        ...
                                        @endphp
                                        <button type="button" title="View Details" data-feedback="{{ json_encode($feedbackData) }}" onclick="openFeedbackDetail(this)" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors cursor-pointer">
                                            <x-heroicon-o-eye class="w-5 h-5" />
                                        </button>
                                        
                                        @if($fb->status === 'Waiting')
                                            <button type="button" title="Mark as Completed" onclick="openCompleteModal('{{ $fb->id }}')" class="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-lg transition-colors cursor-pointer">
                                                <x-heroicon-o-check-circle class="w-5 h-5" />
                                            </button>
                                        @else
                                            <button type="button" title="Completed" disabled class="p-1.5 text-green-500 opacity-50 cursor-not-allowed">
    ...
    <div id="completeFeedbackModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4">
        <div class="bg-white dark:bg-[#242424] rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col border border-transparent dark:border-gray-700/50">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700/50">
                <h3 class="text-lg font-bold text-gray-800 dark:text-white">Mark as Completed</h3>
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        <div class="bg-yellow-50 p-3 rounded-lg border border-yellow-200 text-sm text-yellow-800 mb-4">
                            Please fill in a reply / remark for this feedback. The reply will be sent to the sender.
                        </div>
                        <div>
                            <label for="remark" class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Remark / Reply <span class="text-red-500">*</span></label>
                            <textarea id="remark" name="remark" rows="3" required placeholder="We have received the feedback and will follow up soon..." class="w-full px-4 py-3 bg-gray-50 dark:bg-[#1A1A1A] border border-gray-200 dark:border-gray-700/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all text-sm resize-none dark:text-white dark:placeholder-gray-500"></textarea>
                        </div>
                    </div>
                </div>
                
                <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700/50 bg-gray-50 dark:bg-[#1A1A1A] flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 text-sm font-bold bg-green-600 hover:bg-green-700 text-white rounded-xl shadow-lg shadow-green-600/20 transition-all flex items-center gap-2">
                        <span class="btn-text">Send Remark & Complete</span>
    ...
    <div id="feedbackDetailDrawer" onclick="if(event.target === this) closeFeedbackDetail()" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm transition-opacity">
        <div class="fixed inset-y-0 right-0 w-full max-w-xl bg-white dark:bg-[#242424] shadow-2xl transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col border-l border-transparent dark:border-gray-700/50" id="drawerContent">
            <!-- Header -->
            <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700/50 flex items-center justify-between bg-white dark:bg-[#242424] z-10 sticky top-0">
                <div>
                    <h3 class="text-xl font-bold text-gray-800 dark:text-white leading-none">Feedback and Suggestions</h3>
                    <div class="mt-2.5">
            ...
            <div class="p-6 overflow-y-auto flex-1 bg-gray-50/50 dark:bg-[#1A1A1A] space-y-5">
                ...
                <div class="flex items-center gap-3.5 pb-4 border-b border-gray-200/80 dark:border-gray-700/50">
                    ...
                    <div>
                        <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Sender</p>
                ...
                <div class="space-y-5">
                    <div>
                        <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Date Sent</p>
                        <p class="font-bold text-gray-800 dark:text-white text-sm" id="detailDate">-</p>
                    </div>
                    
                    <div>
                        <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Feedback Description</p>
                    ...
                    <div id="detailFileContainer" class="hidden pt-1">
                        <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Document / File</p>
                        <div class="flex items-center justify-between p-3 bg-white dark:bg-[#242424] border border-gray-200 dark:border-gray-700/50 rounded-xl shadow-sm">
                            <div class="flex items-center gap-2.5 min-w-0">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[10px] text-gray-400 font-semibold leading-none mt-0.5">Attachment file from member</p>
                                </div>
                            </div>
                        ...
                        <div id="detailImagePreviewContainer" class="mt-3 hidden">
                            <p class="text-[10px] font-semibold text-gray-400 mb-1.5">Image Preview:</p>
                            <div class="border border-gray-200 dark:border-gray-700/50 rounded-xl overflow-hidden max-h-48 bg-white dark:bg-[#1A1A1A] flex items-center justify-center p-2">
                ...
                <div id="detailResponseContainer" class="space-y-3">
                    <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider" id="detailResponseTitle">HRD Reply</p>
                    ...
                    <div id="waitingResponseArea" class="hidden">
                        <div class="p-4 bg-amber-50/30 dark:bg-amber-900/20 border border-amber-100/80 dark:border-amber-700/30 rounded-2xl shadow-sm flex items-start gap-3">
                            ...
                            <p class="text-amber-800 dark:text-amber-300 text-xs font-semibold leading-relaxed">No response yet. Please use the checkmark (✓) button in the action column of the table to provide a response and complete this complaint.</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div id="filterModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4 transition-opacity">
        <div class="bg-white dark:bg-[#242424] rounded-2xl shadow-xl w-full max-w-sm flex flex-col border border-transparent dark:border-gray-700/50">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700/50">
                <h3 class="text-lg font-bold text-gray-800 dark:text-white">Filter Feedbacks</h3>
                ...
                <div class="p-6 space-y-5">
                    <div>
                        ...
                        <div class="relative custom-dropdown-container">
                            <input type="hidden" name="status" id="fbStatusInput" value="{{ request('status', 'All Status') }}">
                            <button type="button" onclick="toggleCustomDropdown('fbStatusMenu')" class="w-full flex items-center justify-between pl-4 pr-3 py-2.5 bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-gray-700/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 transition-colors text-gray-700 dark:text-white cursor-pointer shadow-sm">
                                <span id="fbStatusBtnText">{{ request('status', 'All Status') }}</span>
                            ...
                            <div id="fbStatusMenu" class="custom-dropdown-menu absolute z-50 hidden mt-1.5 w-full bg-white dark:bg-[#242424] border border-gray-100 dark:border-gray-700/50 rounded-xl shadow-lg">
                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-300 font-medium">
                                    <li><button type="button" onclick="selectDropdownOption('fbStatusInput', 'fbStatusBtnText', 'fbStatusMenu', 'All Status', 'All Status')" class="inline-flex items-center w-full px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors">All Status</button></li>
                                    <li><button type="button" onclick="selectDropdownOption('fbStatusInput', 'fbStatusBtnText', 'fbStatusMenu', 'Waiting', 'Waiting')" class="inline-flex items-center w-full px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors">Waiting</button></li>
                                    <li><button type="button" onclick="selectDropdownOption('fbStatusInput', 'fbStatusBtnText', 'fbStatusMenu', 'Completed', 'Completed')" class="inline-flex items-center w-full px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors">Completed</button></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative custom-dropdown-container">
                            ...
                            <button type="button" onclick="toggleCustomDropdown('fbSortMenu')" class="w-full flex items-center justify-between pl-4 pr-3 py-2.5 bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-gray-700/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 transition-colors text-gray-700 dark:text-white cursor-pointer shadow-sm">
                                <span id="fbSortBtnText">{{ request('sort', 'desc') === 'desc' ? 'Newest Data' : 'Oldest Data' }}</span>
                                <svg class="w-4 h-4 text-gray-400 pointer-events-none" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 9-7 7-7-7"/></svg>
                            </button>
                            <div id="fbSortMenu" class="custom-dropdown-menu absolute z-50 hidden mt-1.5 w-full bg-white dark:bg-[#242424] border border-gray-100 dark:border-gray-700/50 rounded-xl shadow-lg">
                                <ul class="py-1 text-sm text-gray-700 dark:text-gray-300 font-medium">
                                    <li><button type="button" onclick="selectDropdownOption('fbSortInput', 'fbSortBtnText', 'fbSortMenu', 'desc', 'Newest Data')" class="inline-flex items-center w-full px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors">Newest Data</button></li>
                                    <li><button type="button" onclick="selectDropdownOption('fbSortInput', 'fbSortBtnText', 'fbSortMenu', 'asc', 'Oldest Data')" class="inline-flex items-center w-full px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition-colors">Oldest Data</button></li>
                                </ul>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function openFeedbackDetail(btn) {
            ...
            document.getElementById('detailResponseTitle').textContent = 'HRD Reply';
            if (feedback.status === 'Completed') {
                ...
                if (feedback.remark) {
                    ...
                    remarkText.textContent = 'Feedback marked as completed without comment.';
